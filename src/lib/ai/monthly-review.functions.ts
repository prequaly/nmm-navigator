...
export const generateMonthlyReview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [
      ...
      supabase
        .from("kpis")
        .select("name,baseline,current_value,target,unit,category")
        .eq("organization_id", data.organizationId),
      supabase
        .from("risks")
        .select("title,likelihood,impact,mitigation")
        .eq("organization_id", data.organizationId)
      ...
      supabase
        .from("grants")
        .select(
          "funder_name,grant_name,status,amount_requested,amount_awarded,probability,application_deadline,end_date",
        )
        .eq("organization_id", data.organizationId),
      supabase
        .from("action_items")
      ...
      supabase
        ...
        .lte("application_deadline", next30.toISOString().slice(0, 10))
        .order("application_deadline"),
    ]);

    // Monthly financial snapshot
    const monthRevenue = (revenue ?? []).reduce(
      (sum, r) => sum + monthAmount(r.yearly_amounts as YearlyAmounts, year, monthIdx),
      0,
    );
    const monthExpense = (expenses ?? []).reduce(
      (sum, e) => sum + monthAmount(e.yearly_amounts as YearlyAmounts, year, monthIdx),
      0,
    );
    ...
    for (let m = 0; m <= monthIdx; m++) {
      ytdRev += (revenue ?? []).reduce(
        (s, r) => s + monthAmount(r.yearly_amounts as YearlyAmounts, year, m),
        0,
      );
      ytdExp += (expenses ?? []).reduce(
        (s, e) => s + monthAmount(e.yearly_amounts as YearlyAmounts, year, m),
        0,
      );
    }
    ...
    const pipelineGrants = (grants ?? []).filter(
      (g) => !["awarded", "active", "declined", "closed"].includes(g.status),
    );
    const pipelineWeighted = pipelineGrants.reduce(
      (s, g) => s + Number(g.amount_requested ?? 0) * (Number(g.probability ?? 0) / 100),
      0,
    );
    const committedAwarded = activeGrants.reduce(
      (s, g) => s + Number(g.amount_awarded ?? g.amount_requested ?? 0),
      0,
    );
    ...
    const ctx = {
      ...
      grants: {
        ...
        deadlinesNext30: (upcomingDeadlines ?? []).map(
          (g) => `${g.funder_name} — ${g.grant_name} (due ${g.application_deadline})`,
        ),
      },
      kpis: kpiStatus,
