...
import { fetchImpactReportData, formatDate } from "@/lib/exports/data";
import { IMPACT_LENSES } from "@/lib/plan/sections";

export const Route = createFileRoute("/_authenticated/report/print/impact-report")({
  head: () => ({ meta: [{ title: "IMPACT Report — Print" }] }),
  component: PrintImpactReport,
});

function PrintImpactReport() {
  const { orgId } = useCurrentOrg();
  const [data, setData] = useState<Awaited<ReturnType<typeof fetchImpactReportData>> | null>(null);

  useEffect(() => {
    if (!orgId) return;
    fetchImpactReportData(orgId).then(setData);
  }, [orgId]);
  ...
  return (
    ...
    >
      <PrintSection title="Assessment Snapshot">
        {assessment ? (
          ...
          <p className="text-slate-400 italic">
            The IMPACT Framework Audit has not been completed yet.
          </p>
        )}
      ...
      <PrintSection title="How Strategic Pillars Map to IMPACT">
        {pillars.length === 0 ? (
          <p className="text-slate-400 italic">No strategic pillars defined yet.</p>
        ) : (
          <div className="space-y-4">
            {IMPACT_LENSES.map((lens) => {
              const matches = pillars.filter((pl) => (pl.impact_lenses ?? []).includes(lens.key));
              return (
                <div key={lens.key}>
                  <p className="font-semibold text-brand-deep">{lens.label}</p>
                  {matches.length === 0 ? (
                    <p className="text-slate-400 italic text-sm">
                      No pillar currently tagged to this lens.
                    </p>
                  ) : (
      ...
      <PrintSection title="Strategic Issues Narrative">
